...
- 通知音は最大30秒・**ループ不可**。そこで同じアラームを短間隔で連続通知して「鳴り続ける（擬似ループ）」を実現します（`kAlarmRepeatCount`/`kAlarmRepeatIntervalSec`）。「止める」またはアプリを開くと残りの連続通知はキャンセルされます。
- 保留中のローカル通知は **64 件まで**。擬似ループは通知を消費するため、**直近 `kLoopAlarmsAhead` 件のアラームだけ**に付け、それ以外は1件です。＝「長く鳴らす」と「先まで予約」はトレードオフ。アプリを開く/復帰するたびに貼り直します。
- サイレント/集中モードで鳴らない場合があります（Time Sensitive で緩和）。
- **アラーム音**（クラシック/リング/アラーム）は同梱。iOSは `ios/Runner/Sounds/*.caf` を Xcode の Runner ターゲットに追加する必要があります（RELEASE_ja.md 参照）。同梱音源は Pixabay の royalty-free 素材（商用可・帰属不要）。
